...
#'   
#' @export
psfmi_coxr_fw <- function(data, nimp, impvar, status, time, p.crit, P, keep.P, method, strata.P)
{
  ...
  strata.P <- P_orig[grepl("strata", P_orig)]
  ...
  for(j in 1:length(P)) {
    ...
    for (k in 1:length(P)) {
      ...
      if(method=="MPR") {
        ...
        fm_terms <- attr(terms(fm), "term.labels")
        if(any(grepl("strata", fm_terms))){
          fm_terms <- fm_terms[!grepl("strata", fm_terms)]
          rownames(med.pvalue) <- clean_P(fm_terms) %>%
            str_replace(":", "*")
        } else {
          rownames(med.pvalue) <- clean_P(fm_terms) %>%
            str_replace(":", "*")
