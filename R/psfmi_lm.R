                     ...
                     direction=NULL)
{
  ...
  if(all(data[Outcome]==1 | data[Outcome]==0))
    stop("Outcome should be a continuous variable")
  if ((nvar <- ncol(data)) < 2)
  ...
  if(p.crit==1){
    pobjpool <-
      psfmi_lm_bw(data = data, nimp=nimp, impvar = impvar, Outcome= Outcome,
  ...
  if(direction=="FW"){
    pobjfw <-
      psfmi_lm_fw(data = data, nimp = nimp, impvar = impvar, Outcome = Outcome, p.crit = p.crit,
