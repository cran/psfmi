...
To internally validate the model we use the `psfmi_` function. With this function five different
...
the fit is satisfactory. This is also confirmed by the calibration plot
where the calibration curves averaged across multiply imputed datasets is 
near the optimal (dashed) line. The slope (regression coefficients) has a 
value of 0.88741 and slightly deviates from the value of 1, which means 
that the coefficient values differ between the development and external dataset.
